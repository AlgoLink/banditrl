...
class dbEngine(object):
    ...
    def format_index(self,index_sql):
    ...
    def append_df(self, _df: str, tbl_name: str, df):
        _df = self.sqlprotect(_df)
        tbl_name = self.sqlprotect(tbl_name)
        with self._session() as s:
            s.execute(f"insert into {tbl_name} select * from {_df}")
            return True
    ...
    def sql(self,sql_script):
        _sql_script = self.format_index(sql_script)
        with self._session() as s:
